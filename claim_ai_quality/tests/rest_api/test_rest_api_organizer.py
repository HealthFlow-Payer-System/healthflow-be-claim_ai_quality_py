...
from claim_ai_quality.tests.rest_api.requests_response_mock import \
    RequestsMocked
from claim_ai_quality.tests.rest_api.utils import \
    ClaimAITestInitialDataGenerator


class TestRestAIEvaluationOrganizer(ClaimAITestInitialDataGenerator, testcases.TestCase, openIMISGraphQLTestCase):
    ...
    def test_direct_graphene_call(self, send, mutation, organizer, requests_mock, url_claim, url_bundle, url_login):
        ...
        expected_request = ClaimBundleConverter(
            ClaimSerializer()).build_claim_bundle(Claim.objects.filter(id=9999))
    ...
    def test_submit_claim_evaluation(self, submit, send, mutation, organizer, requests_mock, url_claim, url_bundle, url_login):
        ...
        executed = self.send_mutation_raw(
            ...
            context=self.BaseTestContext(self._TEST_USER).user_token,
            follow=False
        )
